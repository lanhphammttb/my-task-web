...
import {
  Archive, ArchiveRestore, CalendarClock, Check, ListChecks, MoreHorizontal, Pencil, Plus, Target, Trash2,
} from 'lucide-react';
...
export default function GoalsView({ onEdit, onFocus }: Props) {
  ...
  const [draft, setDraft] = useState({ title: '', description: '', targetDate: '', color: GOAL_COLORS[0] });
  ...
  const openCreate = () => {
    setDraft({ title: '', description: '', targetDate: '', color: GOAL_COLORS[data.goals.length % GOAL_COLORS.length] });
    setEditing(null);
    setFormOpen(true);
  };

  const openEdit = (g: Goal) => {
    setDraft({ title: g.title, description: g.description, targetDate: g.targetDate ?? '', color: g.color });
    setEditing(g);
    setFormOpen(true);
  };
  ...
  const unassigned = sortTasks(data.tasks.filter((t) => !t.goalId && t.status !== 'done'));

  const card = (g: Goal) => {
    const list = tasksByGoal.get(g.id) ?? [];
    const done = list.filter((t) => t.status === 'done').length;
    const ratio = list.length ? done / list.length : 0;
    const daysLeft = g.targetDate ? differenceInCalendarDays(parseKey(g.targetDate), new Date()) : null;
        ...
        <div className="space-y-3 p-4">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              {g.description && <p className="text-muted-foreground mt-0.5 text-xs">{g.description}</p>}
            </div>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="size-7 shrink-0" aria-label="Tuỳ chọn mục tiêu">
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem onClick={() => updateGoal(g.id, { archived: !g.archived })}>
                  {g.archived ? <ArchiveRestore className="size-3.5" /> : <Archive className="size-3.5" />}
                  {g.archived ? 'Kích hoạt lại' : 'Lưu trữ'}
                </DropdownMenuItem>
                <DropdownMenuItem variant="destructive" onClick={() => setPendingDelete(g)}>
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="tabular w-10 text-right text-xs font-bold">{Math.round(ratio * 100)}%</span>
          </div>

          <div className="flex flex-wrap items-center gap-2">
            <MetaChip icon={ListChecks} className={complete ? 'border-success/35 bg-success/12 text-success' : undefined}>
            ...
            {daysLeft !== null && (
              <MetaChip
                icon={CalendarClock}
                className={cn(
                  daysLeft < 0 && 'border-destructive/40 bg-destructive/12 text-destructive',
                  daysLeft >= 0 && daysLeft <= 7 && 'border-warning/40 bg-warning/12 text-warning',
                )}
              >
                {daysLeft < 0 ? `Trễ ${Math.abs(daysLeft)} ngày` : `Còn ${daysLeft} ngày`}
              </MetaChip>
            )}
            {g.targetDate && <MetaChip>{longDate(parseKey(g.targetDate))}</MetaChip>}
            <button
          ...
          {isOpen && (
            <div className="space-y-2 pt-1">
              {list.length === 0 ? (
                <EmptyState icon={Target} title="Chưa gắn nhiệm vụ nào cho mục tiêu này" />
              ) : (
  ...
  return (
    <div className="mx-auto flex w-full max-w-5xl flex-col gap-4">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-muted-foreground max-w-lg text-xs">
            Gắn nhiệm vụ hằng ngày vào mục tiêu lớn để thấy rõ mình đang tiến tới đâu.
          </p>
        </div>
      ...
      {archived.length > 0 && (
        <>
          <h4 className="text-muted-foreground text-[11px] font-bold tracking-wider uppercase">Đã lưu trữ</h4>
          <div className="grid gap-4 opacity-60 lg:grid-cols-2">{archived.map(card)}</div>
        </>
      )}
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="sm:max-w-[520px]">
          <DialogHeader>
            <DialogTitle>{editing ? 'Sửa mục tiêu' : 'Mục tiêu mới'}</DialogTitle>
          </DialogHeader>
              ...
              <div className="flex flex-wrap gap-2">
                {GOAL_COLORS.map((c) => (
                  <button
                    ...
                    style={{ background: c }}
                  >
                    {draft.color === c && <Check className="size-4 text-white" strokeWidth={3} />}
                  </button>
                ))}
              </div>
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={submit} disabled={!draft.title.trim()}>Lưu mục tiêu</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              “{pendingDelete?.title}” sẽ bị xoá. Các nhiệm vụ vẫn giữ nguyên nhưng không còn gắn mục tiêu.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
