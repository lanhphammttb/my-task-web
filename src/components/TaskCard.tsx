...
import {
  CalendarDays, Check, ChevronDown, Clock, Copy, CornerDownRight, Crosshair, Flag,
  ListChecks, Pause, Pencil, Play, Target, Timer, Trash2,
} from 'lucide-react';
import type { Task } from '../types';
import { addDays, countdown, dateKey, formatDuration, parseKey, relativeDay } from '../lib/date';
...
export default function TaskCard({ task, onEdit, onFocus, showDate }: Props) {
  const { toggleDone, setStatus, removeTask, duplicateTask, moveTask, toggleSubtask, data } = useApp();
  ...
  return (
    ...
    >
      ...
      <span aria-hidden className={cn('w-[3px] shrink-0 self-stretch', p.dot)} />
      ...
      <div className="flex min-w-0 flex-1 gap-3 py-3">
        <div className="relative shrink-0">
          ...
          >
            <Check className={cn('size-3.5 transition-opacity', done ? 'opacity-100' : 'opacity-0')} strokeWidth={3.5} />
          </button>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            <MetaChip icon={p.icon} className={p.soft}>{p.label}</MetaChip>
            {showDate && <MetaChip icon={CalendarDays}>{relativeDay(task.date)}</MetaChip>}
            {task.startTime && <MetaChip icon={Clock}>{task.startTime}</MetaChip>}
            {task.estimateMin > 0 && <MetaChip icon={Timer}>{formatDuration(task.estimateMin)}</MetaChip>}
            {task.focusMin > 0 && (
              <MetaChip icon={Crosshair} className="border-success/35 bg-success/12 text-success">
                {formatDuration(task.focusMin)}
              </MetaChip>
            )}
            {cd.label && <MetaChip icon={Flag} className={DEADLINE_CLASS[cd.level]}>{cd.label}</MetaChip>}
            {goal && (
              <MetaChip
                icon={Target}
                style={{ color: goal.color, borderColor: `${goal.color}59`, backgroundColor: `${goal.color}1f` }}
              >
                {goal.title}
              </MetaChip>
            )}
            {task.subtasks.length > 0 && (
              <MetaChip icon={ListChecks} className={subDone === task.subtasks.length ? 'text-success' : undefined}>
                {subDone}/{task.subtasks.length}
              </MetaChip>
            )}
            {task.recurrence !== 'none' && (
              <MetaChip icon={RECURRENCE_ICON}>{RECURRENCE_UI[task.recurrence]}</MetaChip>
            )}
            {task.tags.map((tag) => (
              <MetaChip key={tag} className="border-primary/30 bg-primary/12 text-primary">#{tag}</MetaChip>
            ))}
          </div>
            ...
            {open && (
              ...
              >
                <div className="border-border/70 mt-3 space-y-3 border-t border-dashed pt-3">
                  {task.note && <p className="text-muted-foreground text-xs whitespace-pre-wrap">{task.note}</p>}
                          ...
                          >
                            <span
                              className={cn(
                                ...
                                s.done ? 'border-success bg-success text-white' : 'border-muted-foreground/40',
                              )}
                            >
                              {s.done && <Check className="size-2.5" strokeWidth={4} />}
                            </span>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {!done && (
                      <Button
                        ...
                        onClick={() => setStatus(task.id, task.status === 'doing' ? 'todo' : 'doing')}
                      >
                        {task.status === 'doing' ? <Pause className="size-3.5" /> : <Play className="size-3.5" />}
                        {task.status === 'doing' ? 'Tạm dừng' : 'Bắt đầu'}
                      </Button>
                    )}
                    {onFocus && !done && (
                      <Button size="sm" variant="outline" className="h-7 gap-1.5 px-2.5 text-xs" onClick={() => onFocus(task)}>
                        <Crosshair className="size-3.5" /> Tập trung
                      </Button>
                    )}
                    <Button size="sm" variant="outline" className="h-7 gap-1.5 px-2.5 text-xs" onClick={() => onEdit(task)}>
                    ...
                    <Button
                      ...
                      variant="outline"
                      className="h-7 gap-1.5 px-2.5 text-xs"
                      onClick={() => moveTask(task.id, dateKey(addDays(parseKey(task.date), 1)))}
                    >
                      <CornerDownRight className="size-3.5" /> Dời sang mai
                    </Button>
                    <Button size="sm" variant="outline" className="h-7 gap-1.5 px-2.5 text-xs" onClick={() => duplicateTask(task.id)}>
