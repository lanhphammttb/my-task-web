...
const blank = (date: string): Draft => ({
  title: '', note: '', date, startTime: '', deadline: '',
  priority: 'medium', estimateMin: '30', goalId: 'none', tags: '', recurrence: 'none', subtasks: [],
});

export default function TaskEditorDialog({ open, task, defaultDate, onOpenChange }: Props) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] gap-0 overflow-y-auto sm:max-w-[640px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Càng cụ thể càng dễ bắt tay vào làm. Đặt hạn chót để app nhắc bạn đúng lúc.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 py-5 sm:grid-cols-2">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      active ? meta.soft : 'border-border text-muted-foreground hover:bg-muted',
          ...
          <div className="grid gap-2">
            ...
            <Input id="task-date" type="date" value={draft.date} onChange={(e) => set('date', e.target.value)} />
          </div>

          <div className="grid gap-2">
            <Label htmlFor="task-start">Giờ bắt đầu</Label>
            <Input id="task-start" type="time" value={draft.startTime} onChange={(e) => set('startTime', e.target.value)} />
          </div>
            ...
            <Select value={draft.recurrence} onValueChange={(v) => set('recurrence', v as Recurrence)}>
              ...
              <SelectContent>
                {(Object.keys(RECURRENCE_UI) as Recurrence[]).map((r) => (
                  <SelectItem key={r} value={r}>{RECURRENCE_UI[r]}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="grid gap-2">
            ...
            <Select value={draft.goalId} onValueChange={(v) => set('goalId', v)}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">Không thuộc mục tiêu nào</SelectItem>
                {data.goals.filter((g) => !g.archived).map((g) => (
          ...
          <div className="grid gap-2 sm:col-span-2">
            ...
            <Input id="task-tags" value={draft.tags} onChange={(e) => set('tags', e.target.value)} placeholder="công việc, gấp" />
          </div>
          ...
          <div className="grid gap-2 sm:col-span-2">
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                ...
                />
                <Button variant="outline" size="icon" aria-label="Thêm bước" onClick={addSub}>
