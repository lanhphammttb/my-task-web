          ...
          <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  Tốn {next.cost} linh thạch. Linh điền lên {next.plots} ô (đang
                  có {cur.plots}), tay nghề luyện đan +
                  {Math.round(next.refineBonus * 100)}% (đang là +
                  {Math.round(cur.refineBonus * 100)}%). Đây là khoản tiêu không
                  mất đi.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
                <AlertDialogCancel>Thôi</AlertDialogCancel>
                <AlertDialogAction onClick={() => upgradeCave()}>
                  Mở rộng
                </AlertDialogAction>
              </AlertDialogFooter>
            </AlertDialogContent>
    ...
    >
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        <MetaChip>
          Luyện đan{" "}
          {cur.refineBonus > 0
            ? `+${Math.round(cur.refineBonus * 100)}%`
            : "chưa thêm gì"}
        </MetaChip>
      </div>
      ...
      <ol className="mt-3 grid gap-1">
        {CAVE_LEVELS.map((lv) => (
          ...
          >
            ...
            {lv.cost > 0 && (
              <span className="tabular ml-auto">{lv.cost} linh thạch</span>
            )}
          </li>
        ))}
      </ol>
