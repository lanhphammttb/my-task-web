...
describe("Ứng dụng web", () => {
  ...
  it("khởi động vào hub tu luyện với HUD, châm ngôn và hai cột icon", () => {
    ...
    expect(
      screen.getByRole("navigation", { name: "Hoạt động tu luyện" }),
    ).toBeDefined();
    expect(
      screen.getByRole("navigation", { name: "Đạo thể và tài nguyên" }),
    ).toBeDefined();
    expect(
  ...
  it("mở bảng Hôm nay từ thanh tab và thấy dữ liệu mẫu", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Nhật khoá hôm nay" }),
    ).toBeDefined();
    expect(
      (await screen.findAllByText("Chốt tài liệu bàn giao module thanh toán"))
        .length,
    ).toBeGreaterThan(0);
  });
  ...
  it("thêm nhiệm vụ qua ô thêm nhanh và lưu vào localStorage", async () => {
    ...
    fireEvent.change(input, {
      target: { value: "Nhiệm vụ kiểm thử !cao @08:15 ~45 #test" },
    });
    ...
    const saved = JSON.parse(
      localStorage.getItem("my-task-planner/v1") ?? "{}",
    );
    const added = saved.tasks.find(
      (t: { title: string }) => t.title === "Nhiệm vụ kiểm thử",
    );
  ...
  it("đánh dấu hoàn thành làm tăng số việc đã xong", async () => {
    ...
    const before = JSON.parse(
      localStorage.getItem("my-task-planner/v1") ?? '{"tasks":[]}',
    );
    const doneBefore = before.tasks.filter(
      (t: { status: string }) => t.status === "done",
    ).length;

    fireEvent.click(
      (await screen.findAllByLabelText("Đánh dấu hoàn thành"))[0],
    );

    const after = JSON.parse(
      localStorage.getItem("my-task-planner/v1") ?? '{"tasks":[]}',
    );
    const doneAfter = after.tasks.filter(
      (t: { status: string }) => t.status === "done",
    ).length;
  ...
  it("chuyển được sang bảng Tháng, Mục tiêu và Thống kê", async () => {
    ...
    openPanel("Tháng");
    expect(
      await screen.findByRole("heading", { name: "Kế hoạch tháng" }),
    ).toBeDefined();

    openPanel("Mục tiêu");
    expect(
      await screen.findByRole("heading", { name: "Đại nguyện" }),
    ).toBeDefined();
    expect(
      (await screen.findAllByText("Bàn giao dự án Q3")).length,
    ).toBeGreaterThan(0);
  ...
  it("mở được bảng Tiên Lộ với bậc thang cảnh giới và kỳ ngộ", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Tiên lộ" }),
    ).toBeDefined();
  ...
  it("icon Linh Thú mở Động Phủ đúng mục linh thú", async () => {
    ...
    expect(
      await screen.findByRole("heading", { name: "Động phủ" }),
    ).toBeDefined();
  ...
  it("tìm kiếm mở bảng tra cứu và lọc đúng nhiệm vụ theo tên", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/Tìm nhiệm vụ/), {
      target: { value: "chạy bộ" },
    });

    expect(
      await screen.findByRole("heading", { name: "Kết quả tìm kiếm" }),
    ).toBeDefined();
  ...
  it("không cho hoàn thành nhiệm vụ của ngày mai, không bung hiệu ứng", async () => {
    ...
    const done = saved.tasks.filter(
      (t: { date: string; status: string }) =>
        t.date === tomorrow && t.status === "done",
    );
    expect(done).toHaveLength(0);
    // Không có bản ghi nào được thêm vào sổ ghi cho hành động bị chặn.
    expect(
      saved.ledger.every(
        (e: { kind: string }) => e.kind === "task" || e.kind === "session",
      ),
    ).toBe(true);
  });
