...
export function AppProvider({ children }: { children: ReactNode }) {
  const [data, setData] = useState<AppData>(() => {
    ...
    const base =
      loaded.tasks.length === 0 && loaded.goals.length === 0
        ? seedData()
        : loaded;
  ...
  const detectMilestones = useCallback(
    (before: AppData, after: AppData, dayKey: string) => {
      ...
      if (
        !isPerfectDay(before.tasks, dayKey) &&
        isPerfectDay(after.tasks, dayKey)
      ) {
        found.push({
          kind: "perfect-day",
          count: after.tasks.filter((t) => t.date === dayKey).length,
        });
      }
    ...
    (id, p) =>
      patch((d) => {
        ...
        const needsResign =
          ...
          !!after &&
          after.status === "done" &&
          taskValue(before) !== taskValue(after);

        const ledger = needsResign
          ? appendEntry(
              dropEntries(d.ledger, "task", id),
              "task",
              id,
              taskValue(after),
            )
          ...
          const exists =
            ...
            d.tasks.some(
              (t) =>
                ...
                t.recurrence === target.recurrence,
            );
          if (next && !exists) {
            tasks = [
              ...tasks,
              {
                ...
                deadline: target.deadline
                  ? `${next}T${target.deadline.slice(11)}`
                  : undefined,
  ...
  const pushOverdueToToday = useCallback<Ctx["pushOverdueToToday"]>(() => {
    ...
    const moved = data.tasks.filter(
      (t) => t.status !== "done" && t.date < today,
    );
    if (moved.length) {
      patch((d) => ({
        ...d,
        tasks: d.tasks.map((t) =>
          t.status !== "done" && t.date < today ? { ...t, date: today } : t,
        ),
      }));
  ...
  const clearDone = useCallback<Ctx["clearDone"]>(
    (before) => {
      ...
      const victims = data.tasks.filter(
        (t) => t.status === "done" && t.date < cutoff,
      );
      if (victims.length) {
        patch((d) => {
          ...
          return {
            ...d,
            tasks,
            ledger: rebuildLedger({ tasks, sessions: d.sessions }),
          };
        });
  ...
  const updateGoal = useCallback<Ctx["updateGoal"]>(
    (id, p) =>
      patch((d) => ({
        ...d,
        goals: d.goals.map((g) => (g.id === id ? { ...g, ...p } : g)),
      })),
    [patch],
  );
  ...
  const logSession = useCallback<Ctx["logSession"]>(
    (minutes, taskId) => {
      ...
      patch((d) => ({
        ...
        tasks: taskId
          ? d.tasks.map((t) =>
              t.id === taskId ? { ...t, focusMin: t.focusMin + minutes } : t,
            )
          : d.tasks,
          ...
          case "pill":
            return outcome.pill
              ? {
                  ...
                  pills: {
                    ...d.pills,
                    [outcome.pill]: d.pills[outcome.pill] + 1,
                  },
                }
              : d;
          default:
  ...
  const summon = useCallback<Ctx["summon"]>(() => {
    ...
    patch((d) => ({
      ...
      beasts: duplicate
        ? d.beasts.map((b) =>
            b.id === beast.id ? { ...b, fed: b.fed + DUPLICATE_FEED } : b,
          )
        : [
            ...d.beasts,
            { id: beast.id, fed: 0, obtainedAt: new Date().toISOString() },
          ],
  ...
  const feedBeast = useCallback<Ctx["feedBeast"]>(
    (id) => {
      ...
      patch((d) => ({
        ...
        beasts: d.beasts.map((b) =>
          b.id === id ? { ...b, fed: b.fed + FEED_GAIN } : b,
        ),
      }));
      return true;
    },
